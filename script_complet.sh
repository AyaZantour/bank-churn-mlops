...
LOCATION="francecentral"   # Forcé France Central
...
echo "📦 Vérification extension containerapp..."
...
if ! az extension show --name containerapp >/dev/null 2>&1; then
    ...
    # Mise à jour silencieuse (fix: removed -y)
    az extension update --name containerapp --only-show-errors 2>/dev/null || true
fi
...
# Fix: remove \r from login server
